...
function loginPage(error = ""): Response {
  ...
  const html = `<!doctype html><html lang="de" dir="ltr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SHAMS Login</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:radial-gradient(circle at 20% 10%,#263d53 0,transparent 26rem),linear-gradient(135deg,#07111f,#111827 50%,#050816);font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;color:#f8fafc}.card{width:min(92vw,450px);padding:34px;border:1px solid rgba(255,255,255,.16);border-radius:30px;background:rgba(8,13,25,.82);box-shadow:0 24px 80px rgba(0,0,0,.45);backdrop-filter:blur(18px)}.brand{display:grid;justify-items:center;gap:18px;margin-bottom:24px;text-align:center}.logo{width:min(260px,74vw);height:auto;display:block;object-fit:contain}.eyebrow{color:#efb58e;font-size:12px;letter-spacing:2px;margin:0 0 5px;text-transform:uppercase}.title{font-size:28px;line-height:1.1;font-weight:850;margin:0}.hint{color:#b8c4d8;line-height:1.6;margin:0 0 26px;text-align:center}label{display:block;color:#dbeafe;font-size:14px;font-weight:700;margin-bottom:8px}input{width:100%;box-sizing:border-box;border:1px solid rgba(255,255,255,.18);border-radius:16px;background:rgba(2,6,23,.66);padding:15px 16px;color:#fff;font-size:17px;outline:none}input:focus{border-color:#efb58e;box-shadow:0 0 0 4px rgba(239,181,142,.18)}button{width:100%;margin-top:16px;border:0;border-radius:16px;padding:15px 18px;background:linear-gradient(135deg,#efb58e,#f5d09e);color:#111827;font-size:16px;font-weight:850;cursor:pointer}.error{border-radius:14px;background:rgba(248,113,113,.16);color:#fecaca;padding:12px 14px;margin:0 0 16px;font-size:14px}</style></head><body><main class="card"><div class="brand"><img class="logo" src="/shams-logo.jpg" alt="SHAMS Bakery Operations"><div><p class="eyebrow">SHAMS Hamburg</p><h1 class="title">Admin Zugang</h1></div></div><p class="hint">Bitte Passwort eingeben, um das Bäckerei-System zu öffnen.</p>${errorHtml}<form method="post" action="/login"><label for="password">Passwort</label><input id="password" name="password" type="password" autocomplete="current-password" autofocus required><button type="submit">Einloggen</button></form></main></body></html>`;
